...
import com.example.lingualy.databinding.RowLeaderboardsBinding;
...
public class LeaderboardsAdapter extends RecyclerView.Adapter<LeaderboardsAdapter.LeaderboardViewHolder>{
    ...
    public void onBindViewHolder(@NonNull LeaderboardViewHolder holder, int position) {
        ...
        holder.binding.name.setText(user.getUname());
        holder.binding.coins.setText(String.valueOf(user.getCoins()));
        holder.binding.index.setText(String.format("#%d", position+1));

        Glide.with(context).load(user.getProfile()).into(holder.binding.imageView5);
    }
